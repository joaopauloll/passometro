    ...
    <>
      ...
      <div className="no-print print-header pb-6 flex items-center justify-between">
        <div>
          ...
          <h1 className="text-2xl font-bold text-gray-900 mt-2">Calendário de Tratamento</h1>
          <p className="text-gray-500 text-sm mt-1">Clique em "Imprimir" para gerar a folha para o paciente</p>
        </div>
        <div className="flex items-center gap-2">
          <PrintButton />
          <DownloadPDFButton nomeArquivo={`calendario-${paciente.nome.replace(/\s+/g, '-')}`} />
        </div>
      </div>
      ...
      <div className="print-page bg-white border border-gray-200 rounded-xl p-8 shadow-sm">
        ...
        <div className="border-b-2 border-blue-600 pb-4 mb-6">
          <div className="flex justify-between items-start">
            <div>
              <h2 className="text-xl font-bold text-blue-700">CALENDÁRIO DE TRATAMENTO</h2>
              {cirurgioes.length > 0 && (
                <p className="text-sm font-medium mt-1">Dr(a). {cirurgioes.join(' / Dr(a). ')}</p>
              )}
            </div>
        ...
        <div className="grid grid-cols-2 gap-x-8 gap-y-1 mb-6 text-sm">
          <div><span className="font-semibold">Paciente:</span> {paciente.nome}</div>
          <div><span className="font-semibold">Registro:</span> {paciente.registroHospitalar}</div>
          <div className="col-span-2"><span className="font-semibold">Diagnóstico:</span> {paciente.diagnostico}{paciente.cid ? ` (${paciente.cid})` : ''}</div>
          {primeiraCirurgia && (
            <div className="col-span-2"><span className="font-semibold">Cirurgia realizada:</span> {primeiraCirurgia.nomeCirurgia}</div>
          )}
        </div>

        {/* Linha do tempo */}
        <div className="mb-8">
          <h3 className="font-bold text-sm uppercase text-gray-500 mb-4">Linha do Tempo</h3>
            ...
            <div className="space-y-4">
              ...
              {paciente.traumaData && (
                <div className="flex items-start gap-4">
                  ...
                  <div className="pt-1">
                    ...
                    <p className="text-sm text-gray-600">{format(new Date(paciente.traumaData), "dd/MM/yyyy")}</p>
                    {paciente.traumaMecanismo && <p className="text-xs text-gray-500">{paciente.traumaMecanismo}</p>}
                  </div>
                </div>
              )}
              ...
              <div className="flex items-start gap-4">
                ...
                <div className="pt-1">
                  ...
                  <p className="text-sm text-gray-600">{format(new Date(paciente.dataInternacao), "dd/MM/yyyy")}</p>
                </div>
              </div>
              ...
              {paciente.cirurgias.map((c, idx) => (
                <div key={c.id} className="flex items-start gap-4">
                  ...
                  <div className="pt-1">
                    <p className="font-semibold text-sm">Cirurgia {paciente.cirurgias.length > 1 ? idx + 1 : ''}</p>
                    <p className="text-sm text-gray-600">{format(new Date(c.dataCirurgia), "dd/MM/yyyy")}</p>
                    <p className="text-xs text-gray-500">{c.nomeCirurgia} — Dr(a). {c.cirurgiao}</p>
                  </div>
                </div>
              ))}
              ...
              <div className="flex items-start gap-4">
                ...
                <div className="pt-1">
                  ...
                  <p className="text-sm text-gray-600">{format(new Date(), "dd/MM/yyyy")}</p>
                  <p className="text-xs text-gray-500">{diasInternado} dias de internamento</p>
                </div>
              </div>
        ...
        <div className="mb-6 p-4 bg-blue-50 rounded-lg border border-blue-200">
          <h3 className="font-bold text-sm uppercase text-blue-700 mb-2">Raio-X de Retorno</h3>
          <div className="text-sm text-gray-700">
            <p>☐ Sim — Incidências: <span className="border-b border-gray-400 inline-block min-w-48">______________________________</span></p>
        ...
        <div className="mb-6">
          <h3 className="font-bold text-sm uppercase text-gray-500 mb-2">Recomendações</h3>
          <div className="space-y-1 text-sm">
            <p>• Retornar ao ambulatório na data marcada</p>
            <p>• Em caso de febre, aumento da dor ou alteração no curativo, procurar pronto-socorro</p>
            {paciente.alergias && <p>• <strong>Alergia:</strong> {paciente.alergias}</p>}
          </div>
        </div>
        ...
        <div className="border-t border-gray-200 pt-4 mt-8">
          <div className="flex justify-between items-end">
            <div className="text-sm text-gray-500">
              <p className="text-xs">Documento gerado em {format(new Date(), "dd/MM/yyyy 'às' HH:mm")}</p>
            </div>
