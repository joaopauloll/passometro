...
export default async function RelatorioAltaPage({ params }: Params) {
  ...
  linhas.push(`Data de internação: ${format(new Date(paciente.dataInternacao), "dd/MM/yyyy")}`)
  linhas.push('')
  linhas.push('DIAGNÓSTICO')
  linhas.push(paciente.diagnostico + (paciente.cid ? ` (${paciente.cid})` : ''))
  linhas.push('')
  ...
  if (paciente.cirurgias.length > 0) {
    ...
    for (const c of paciente.cirurgias) {
      linhas.push(`• ${c.nomeCirurgia} — ${format(new Date(c.dataCirurgia), "dd/MM/yyyy")} — Dr(a). ${c.cirurgiao}${c.hospitalExterno ? ` (${c.hospitalExterno})` : ''}`)
    }
    linhas.push('')
  }
  ...
  if (paciente.evolucoes.length > 0) {
    ...
    const totalDias = Math.round(
      (new Date().getTime() - new Date(paciente.dataInternacao).getTime()) / (1000 * 60 * 60 * 24)
    )
    linhas.push(`Paciente internado há ${totalDias} dias, evoluindo para alta hospitalar.`)
    if (ultimaEvolucao?.textoGerado) {
      linhas.push(`Última evolução: ${ultimaEvolucao.textoGerado}`)
    }
    linhas.push('')
  }
  ...
  if (ultimaEvolucao) {
    ...
    if (ultimaEvolucao.estavel === true) condicoes.push('estável hemodinamicamente')
    if (ultimaEvolucao.semDor === true || ultimaEvolucao.dorControlada === true) condicoes.push('dor controlada')
    if (ultimaEvolucao.febre === false) condicoes.push('afebril')
    if (condicoes.length > 0) {
      linhas.push(`Paciente recebeu alta em bom estado geral, ${condicoes.join(', ')}.`)
    } else {
  ...
  linhas.push('• Retornar em caso de febre, piora da dor ou alteração no curativo')
  ...
  if (cirurgioes.length > 0) {
    linhas.push(`Cirurgião responsável: Dr(a). ${cirurgioes.join(', Dr(a). ')}`)
  }
    ...
    <div className="max-w-3xl mx-auto">
      <div className="mb-6 flex items-center justify-between">
        <div>
          ...
          <h1 className="text-2xl font-bold text-gray-900 mt-2">Relatório Médico de Alta</h1>
          <p className="text-gray-500 text-sm mt-1">Texto gerado automaticamente — revise antes de usar</p>
        </div>
      ...
      {paciente.fotos.length > 0 && (
        <div className="space-y-6">
          {(['RADIOGRAFIA', 'LESAO_PELE'] as const).map(tipo => {
            ...
            return (
              <div key={tipo} className="bg-white border border-gray-200 rounded-xl p-6 shadow-sm">
                <h3 className="text-sm font-semibold text-slate-700 mb-4">
                  {tipo === 'RADIOGRAFIA' ? '🩻 Radiografias' : '🩹 Lesões de pele'}
                </h3>
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                  {fotos.map(f => (
                    <div key={f.id} className="space-y-1">
                      <img src={f.url} alt={f.descricao || tipo} className="w-full rounded-lg border border-slate-200 object-cover" />
                      {f.dataFoto && (
                        <p className="text-xs text-slate-500 text-center">
                          {format(f.dataFoto, "dd/MM/yyyy", { locale: ptBR })}
                        </p>
                      )}
                      {f.descricao && <p className="text-xs text-slate-400 text-center truncate">{f.descricao}</p>}
                    </div>
                  ))}
                </div>
              </div>
      ...
      <p className="text-xs text-gray-400 mt-4 text-center">
        Este relatório é gerado automaticamente com base nos dados cadastrados. Revise e complemente conforme necessário antes de assinar.
      </p>
    </div>
  )
}
