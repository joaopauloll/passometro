...
export default async function PacienteDetailPage({ params }: Params) {
  ...
  if (paciente.dataNascimento) {
    idadePaciente = differenceInYears(new Date(), new Date(paciente.dataNascimento))
  }
  ...
  const evolucoesSerialized = paciente.evolucoes.map(e => ({
    ...
    pendencias: e.pendencias.map(p => ({
      id: p.id, descricao: p.descricao, tipo: p.tipo, concluida: p.concluida,
    ...
    <div className="mx-auto">
      ...
      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4 mb-5">
        <div>
          <div className="flex items-center gap-3 flex-wrap">
            ...
            {paciente.temInfeccao && <Badge variant="destructive">Infecção</Badge>}
            {paciente.temAlergia && paciente.alergias && (
              <Badge className="bg-red-100 text-red-800 border-red-300 border">⚠ {paciente.alergias}</Badge>
            )}
          </div>
          <div className="flex items-center gap-2 mt-1 text-sm text-slate-500 flex-wrap">
            <span>Leito <strong className="text-slate-700">{paciente.leito}</strong></span>
            <span>·</span>
            <span>Reg. <strong className="text-slate-700">{paciente.registroHospitalar}</strong></span>
            {paciente.cpf && <><span>·</span><span>CPF <strong className="text-slate-700">{paciente.cpf}</strong></span></>}
            <span>·</span>
            <span><strong className="text-slate-700">{diasInternado}</strong>d internado</span>
            {idadePaciente !== null && <><span>·</span><span><strong className="text-slate-700">{idadePaciente}</strong> anos</span></>}
          </div>
        </div>
      ...
      <PacienteDetailTabs
        ...
        cirurgias={paciente.cirurgias.map(c => ({
          id: c.id, nomeCirurgia: c.nomeCirurgia, cirurgiao: c.cirurgiao,
          dataCirurgia: c.dataCirurgia.toISOString(), hospitalExterno: c.hospitalExterno,
        }))}
        pareceres={paciente.pareceres.map(p => ({
          id: p.id, especialidade: p.especialidade,
          data: p.data.toISOString(), descricao: p.descricao, medico: p.medico,
        }))}
        fotos={paciente.fotos.map(f => ({
          id: f.id, tipo: f.tipo, url: f.url,
          dataFoto: f.dataFoto?.toISOString() ?? null, descricao: f.descricao,
        }))}
