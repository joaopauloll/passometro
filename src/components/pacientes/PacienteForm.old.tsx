...
export default function PacienteForm({ inicial, modo }: Props) {
  ...
  const [form, setForm] = useState<PacienteFormValues>({
    ...
    compSolturaAssetica: (inicial as PacienteFormValues)?.compSolturaAssetica || false,
    compLuxacao: (inicial as PacienteFormValues)?.compLuxacao || false,
    compFalhaImplante: (inicial as PacienteFormValues)?.compFalhaImplante || false,
    compPseudoartrose: (inicial as PacienteFormValues)?.compPseudoartrose || false,
    compOutro: (inicial as PacienteFormValues)?.compOutro || '',
    traumaMecanismo: (inicial as PacienteFormValues)?.traumaMecanismo || '',
    traumaData: (inicial as PacienteFormValues)?.traumaData?.split?.('T')?.[0] || '',
    traumaTempo: (inicial as PacienteFormValues)?.traumaTempo || '',
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6 max-w-3xl mx-auto">
      ...
      <Card>
        <CardHeader className="pb-3">
          <CardTitle className="text-base">Complicações</CardTitle>
        </CardHeader>
        <CardContent className="space-y-3">
          {[
            ...
            <div key={item.key} className="flex items-center gap-2">
              <Checkbox
                id={item.key}
                checked={Boolean(form[item.key as keyof PacienteFormValues])}
                onCheckedChange={(v) => setForm({ ...form, [item.key]: Boolean(v) })}
              />
              <Label htmlFor={item.key} className="cursor-pointer">{item.label}</Label>
            </div>
          ))}
          <div className="space-y-1.5">
            <Label htmlFor="compOutro">Outro</Label>
            <Input
              id="compOutro"
              value={form.compOutro}
              onChange={(e) => setForm({ ...form, compOutro: e.target.value })}
              placeholder="Descrever outra complicação…"
            />
          </div>
        </CardContent>
      </Card>
        ...
        <CardContent className="grid gap-4 sm:grid-cols-2">
          <div className="sm:col-span-2 space-y-1.5">
            ...
            <Input
              ...
              onChange={(e) => setForm({ ...form, traumaMecanismo: e.target.value })}
              placeholder="Ex: Queda da própria altura, acidente de trânsito…"
            />
          </div>
          <div className="space-y-1.5">
            ...
            <Input
              ...
              onChange={(e) => setForm({ ...form, traumaData: e.target.value })}
            />
          </div>
          <div className="space-y-1.5">
            ...
            <Input
              ...
              onChange={(e) => setForm({ ...form, traumaTempo: e.target.value })}
