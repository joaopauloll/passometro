...
export default function PendenciasTab({
  ...
  function isPendenciaAtiva(descricao: string) {
    return pendencias.some((p) => !p.concluida && p.descricao === descricao);
  }
  ...
  async function togglePendencia(pendenciaId: string, concluida: boolean) {
    ...
    try {
      ...
      setPendencias((prev) =>
        prev.map((p) => (p.id === pendenciaId ? atualizada : p)),
      );
  ...
  async function adicionarPendenciaPadrao(item: PendenciaPadrao) {
  ...
  return (
    <Card
      className={`border ${
        abertas.length > 0 ? "border-amber-200" : "border-slate-200"
      }`}
    >
                ...
                <div className="space-y-1.5">
                  {abertas.map((p) => (
                    ...
                    >
                      ...
                      <span
                        className={`shrink-0 rounded border px-1.5 py-0.5 text-[9px] font-medium ${corPendencia(
                          p.tipo,
                        )}`}
                      >
